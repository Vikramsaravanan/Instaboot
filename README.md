...
A full-stack AI chatbot application that generates software install scripts and answers questions from uploaded documents using a multiagent pipeline.

**Stack:** React.js · Tailwind CSS · Node.js (Express) · PostgreSQL + pgvector · @xenova/transformers (all-MiniLM-L6-v2)

---

## Features

- **Script Generator Agent** – Detects install intent from natural language ("install Docker on Windows 11") and returns real, working terminal commands for Windows (winget), macOS (Homebrew), Ubuntu/Debian (apt-get), and Fedora/RHEL (dnf)
- **Knowledge Query Agent** – Embeds uploaded CSV/JSON files and answers questions using semantic similarity search (pgvector)
- **General Chat Agent** – Handles greetings and general questions
- **Version Check Agent** – Fetches live version info from official sources (Node.js, Python) with static fallbacks for all other software
...
- **PostgreSQL 14+** with the **pgvector** extension installed
  - OR Docker Desktop (for the Docker Compose setup)
...
```bash
psql -U postgres -d multiagent_db -f backend/migrations/init.sql
```
...
| `EMBEDDING_MODEL` | `Xenova/all-MiniLM-L6-v2`                                | HuggingFace model for text embeddings             |
